...
This packages comes with an included example (a subset with 96 Chips from GEO accession: GSE65907). This data can be found in `r paste0(path.package("HT12ProcessoR"), "/", "extdata")`. For the purpose of this exercise, copy the folder extdata/ into a new directory, e.g. "~/HT12PreproVignette/extdata/". Additionally, create an empty file called ".here" (just an empty text file) in the base folder of your project. In my example this would be ~/HT12PreproVignette/. We will need this file to set relative paths. 
...
my.con_f <- c(here("extdata/Run12_ControlProbeProfile_590-651.txt"),
              here("extdata/ControlProbeProfile_HSS106-HSS155.txt"))

# gene probe file
my.nobkgd_f <- c(here("extdata/Run12_nonorm_nobkgd_590-651.txt"),
              here("extdata/Einzelanalyse_nonorm_nobkgd_HSS106-HSS155.txt"))

# sample file
my.sample_f <- c(here("extdata/Run12_SamplesTable_590-651.txt"),
              here("extdata/SamplesTable_HSS106-HSS155.txt"))
...
```{r parameter.file}
myparams <- read.delim(here("extdata/input_parameter_010.txt"),
                       as.is = T)
```
...
myparamfile <- here("extdata", "input_parameter_010.txt")
...
renaming_fn <- here("extdata", "renamingsamples.txt")
