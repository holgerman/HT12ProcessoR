runMAtrixEQTLAnova <- function(eset, output_file_name, myesetspalte, mycovarspalte = NULL) {
  ...
  # eset = total_nobkgd_eset_ql; output_file_name = NULL; myesetspalte = "Sentrix.Barcode";  mycovarspalte = "subgroup"
    ...
    if (length(mycovarspalte) > 0) {
        ...
        vgl1
        check1 = identical(round(vgl1$`F value`[1], 5), round(sentrix_vor_combat_pvals$statistic[4], 5))
        stopifnot(check1 & (is.na(vgl1$`F value`[1]) == F))
        ...
        vgl2
        check2 = identical(round(vgl2$`F value`[1], 5), round(sentrix_vor_combat_pvals$statistic[nrow(sentrix_vor_combat_pvals)], 5))
        stopifnot(check2 & (is.na(vgl2$`F value`[1]) == F))
        if (check1 & check2) 
            message("For two examples, R ANOVA and MatrixEQTL in  ANOVA without Covariate are identical. Great.")
    }
